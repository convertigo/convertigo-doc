...
metadesc: Removes a variable/object from the session.    The  Remove from session  step allows to easily remove a value stored in the session using its key.   N
ObjGroup: Sequencer
...
The *Remove from session* step allows to easily remove a value stored in the session using its key.
