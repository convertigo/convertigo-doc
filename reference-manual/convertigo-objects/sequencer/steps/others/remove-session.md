...
metadesc: Defines a step which request the current http session termination.    The  Remove session  step requests the removing of the current HTTP session and 
ObjGroup: Sequencer
ObjCatName: others
...
The *Remove session* step requests the removing of the current HTTP session and all its contexts. No other steps can executed after this one.

**Note:** The creation or the destruction of a http is effective in server mode only.
