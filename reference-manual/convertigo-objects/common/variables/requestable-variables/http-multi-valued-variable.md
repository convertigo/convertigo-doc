...
- In Convertigo Studio, when an *HTTP multi-valued variable* is created in a transaction/sequence, it can be easily replaced by an *HTTP single-valued variable*, using the right-click menu on the variable and choosing the option **Change to** > **SingleValued variable**.
