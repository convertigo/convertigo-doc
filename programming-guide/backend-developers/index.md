...
| Supported Database   | Driver                                                           | Shipped with Convertigo |
|----------------------|------------------------------------------------------------------|-------------------------|
| MariaDB / MySQL      | org.mariadb.jdbc.Driver                                          | yes                     |
| MySQL                | com.mysql.cj.jdbc.Driver (com.mysql.jdbc.Driver for old drivers) | no (license)            |
| PostgreSQL           | org.postgresql.Driver                                            | yes                     |
| Microsoft SQL Server | net.sourceforge.jtds.jdbc.Driver (jTDS)                          | yes                     |
| Microsoft SQL Server | com.microsoft.sqlserver.jdbc.SQLServerDriver (Microsoft driver)  | no                      |
| Oracle               | oracle.jdbc.driver.OracleDriver                                  | no (license)            |
| HSQLDB               | org.hsqldb.jdbcDriver                                            | yes                     |
| IBM DB2 on AS/400    | com.ibm.as400.access.AS400JDBCDriver                             | yes                     |
| IBM DB2              | com.ibm.db2.jcc.DB2Driver                                        | no (license)            |
...
Some Drivers are not shipped with Convertigo due to license restrictions. You will have to provide them in the **/libs** directory of your project (or in the <b>/workspace/lib</b> directory of the Docker image). This is the case for the Oracle Driver, the MySQL Driver, the Microsoft JDBC driver for SQL Server and the DB2 Drivers. Alternatively you can use the MariaDB driver instead of the MySQL driver for MySQL databases. This driver is 100% compatible and avoids licensing issues. The JDBC-ODBC bridge is no longer available in modern Java versions. See <a href="../../operating-guide/appendixes/#sql-drivers-and-related-jar-files">SQL drivers and related jar files</a> for details.
...
Convertigo integrates a Step by Step Sequencer debugger.

Double-click a Sequence to open its interface in the right panel.\
You will see a toolbar with buttons:

![Debug Mode Toolbar](../../images/pguide_img/backend/debug/debug_mode_bar.png)

1. **Debug mode** toggle button: To activate/deactivate debug mode.
2. **Continuous debug mode** button: Stops Step by Step mode and execute all Steps continuously.
3. **Pause the debug process** button: Pauses the **Continuous debug mode**.
4. **Step by step debug mode** button: Click here to execute and reach next Step in Sequence.
5. **Execute again** button: Launches the debug of the Sequence.
6. **Stop the current sequence** button: Stops execution of the Sequence.
7. **XML requester** button: Will output Sequence response ton XML format.
8. **JSON requester** button: Will output Sequence response ton JSON format.
9. **Show the full result** button: In case of big response, it is truncated. Click this button to dysplay full response in viewer.

To debug a Sequence, first step is to click the **Debug mode** button (1).

![Debug Mode Step 1](../../images/pguide_img/backend/debug/debug_mode_step_01.png)

Next step is to click the **Execute again** button (5)

![Debug Mode Step 2](../../images/pguide_img/backend/debug/debug_mode_step_02.png)

Then, to proceed each Steps step by step, you will click the **Step by step debug mode** button. The current Step will be highlighted in green.

![Debug Mode Step 3](../../images/pguide_img/backend/debug/debug_mode_step_03.png)

![Debug Mode Step 4](../../images/pguide_img/backend/debug/debug_mode_step_04.png)

The Sequence response will also be displayed step by step.

![Debug Mode Step 5](../../images/pguide_img/backend/debug/debug_mode_step_05.png)

Finally, when reaching the last Step in Sequence you will get the response in the viewer.

![Debug Mode Step 6](../../images/pguide_img/backend/debug/debug_mode_step_06.png)